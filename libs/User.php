...
class User extends LdapObject
{
    ...
    public function create($username, $firstname, $lastname, $password)
    {
        ...
        if ($error) {
            ...
            $groups = Ldap::getInstance()->search("(cn=*)", __LDAP_GROUP_OU__, ['cn', 'gidnumber']);
            $groupgidnumbers = [];
    ...
    public function remove()
    {
        ...
        if (Ldap::getInstance()->remove($dn)) {
            ...
            return ['RESULT' => true, 'MESSAGE' => 'User deleted.', 'uid' => $this->username];
        }
    }
    ...
    public function setForwardingEmail($emailforward)
    {
        ...
        $data = ["postalAddress" => $emailforward];
        if (Ldap::getInstance()->modify($dn, $data)) {
            ...
            return ['RESULT' => true, 'MESSAGE' => 'Email forwarding set', 'uid' => $this->getUsername()];
        }
    }
    ...
    public function setCrashplan($crashplan)
    {
        ...
        if (Ldap::getInstance()->modify($dn, $data)) {
            ...
            return ['RESULT' => true, 'MESSAGE' => 'Crashplan set', 'uid' => $this->getUsername()];
        }
    }
    ...
    public function setLoginShell($shell)
    {
        ...
        if (Ldap::getInstance()->modify($dn, $data)) {
            ...
            return ['RESULT' => true, 'MESSAGE' => 'Login shell changed.', 'uid' => $this->username];
        } else {
            return [
                'RESULT' => false,
    ...
    public function setHomeSubFolder($subfolder)
    {
        ...
        if (Ldap::getInstance()->modify($dn, $data)) {
            ...
            return ['RESULT' => true, 'MESSAGE' => 'Home Subfolder changed.', 'uid' => $this->username];
        } else {
            return [
                'RESULT' => false,
                'MESSAGE' => 'LDAP Error: ' . Ldap::getInstance()->get_error(),
    ...
    public function addHost($host, $silent = false)
    {
        if (Host::exists($host) && (!$this->getHosts() || !in_array($host, $this->getHosts()))) {
            ...
            $result = Ldap::getInstance()->search($filter, static::$ou, []);

            if ($result['count'] == 0) {
                $data = ["objectClass" => 'account'];
            ...
            if (Ldap::getInstance()->mod_add($dn, $data)) {
                ...
                return ['RESULT' => false, 'MESSAGE' => 'Error: ' . Ldap::getInstance()->get_error()];
            }
        }
    }
    ...
    public function setName($firstname, $lastname)
    {
        $dn = $this->getRDN();
        $name = $firstname . " " . $lastname;
        $data = ["cn" => $name, "sn" => $lastname, "givenName" => $firstname, "gecos" => $name];
        if (Ldap::getInstance()->modify($dn, $data)) {
            Log::info(
            ...
            return ['RESULT' => true, 'MESSAGE' => 'Name successfully changed.', 'uid' => $this->getUsername()];
        }
    }
    ...
    public function setUsername($username)
    {
        ...
        // Change dn of user
        $data = ["mail" => $username . __MAIL_SUFFIX__, "homeDirectory" => "/home/" . $homesub . "/" . $username];
    ...
    public function setPassword($password)
    {
        ...
        if (Ldap::getInstance()->modify($dn, $data)) {
            ...
            return ['RESULT' => true, 'MESSAGE' => 'Password successfully set.', 'uid' => $this->getUsername()];
        } else {
            return ['RESULT' => false, 'MESSAGE' => 'Set Password Failed: ' . Ldap::getInstance()->get_error()];
        }
    }
    ...
    public function lock()
    {
        ...
        if ($result['count'] > 0) {
            ...
            if (Ldap::getInstance()->modify($dn, $data)) {
                ...
                return ['RESULT' => true, 'MESSAGE' => 'User locked.', 'uid' => $this->getUsername()];
            }
        }
        return ['RESULT' => false, 'MESSAGE' => 'Lock failed: ' . Ldap::getInstance()->get_error()];
    }
    ...
    public function unlock()
    {
        ...
        return ['RESULT' => false, 'MESSAGE' => 'Unlock failed: ' . Ldap::getInstance()->get_error()];
    }
    ...
    public function setExpiration($expiration, $reason = "")
    {
        ...
        if (Ldap::getInstance()->modify($dn, $data)) {
            ...
            return ['RESULT' => true, 'MESSAGE' => 'Expiration successfully set.', 'uid' => $this->getUsername()];
        }
    }
    ...
    public function removeExpiration()
    {
        ...
        if (Ldap::getInstance()->mod_del($dn, $data)) {
            ...
            return ['RESULT' => true, 'MESSAGE' => 'Expiration cancelled.', 'uid' => $this->getUsername()];
        }
    }
    ...
    public function setExpirationReason($reason)
    {
        ...
        if ($reason === "") { // Delete
            ...
            $data = ["destinationindicator" => $reason];
            if (Ldap::getInstance()->modify($dn, $data)) {
    ...
    {
        $dn = "uid=" . $this->getUsername() . "," . static::$ou;
        $data = ["facsimiletelephonenumber" => $expiration];
        if (Ldap::getInstance()->modify($dn, $data)) {
        ...
        if ($this->getDescription() != $description) {
            ...
            if ($description == "") { // Delete
                ...
                if (Ldap::getInstance()->mod_del($dn, $data)) {
                    ...
                    return ['RESULT' => true, 'MESSAGE' => 'Removed description', 'uid' => $this->getUsername(),];
                }
            } else { // Update
    ...
    public static function randomPassword($length = 8)
    {
        ...
        } while (!(preg_match("/[A-Z]/u", $password) && preg_match("/[a-z]/u", $password)
                   && preg_match(
                       "/[^A-Za-z]/u",
                       $password
                   )));
    ...
    public static function search(
        ...
        for ($i = 0; $i < $result['count']; $i++) {
            ...
            if ($passwordSet === null
                || strftime("%Y%m%d", $user->getPasswordLastSet()) === strftime(
                    "%Y%m%d",
                    ...
                    if ($userfilter == 'expiring') {
                        ...
                        if ($userfilter == 'expired') {
                            ...
                            if ($userfilter == 'left') {
                                ...
                                if ($userfilter == 'noncampus') {
                                    if ($user->getNonCampus()) {
                                        $users[] = $user;
                                    }
                                } else {
                                    if ($userfilter == 'classroom') {
                                        if ($user->isClassroom()) {
                                            $users[] = $user;
                                        }
                                    } else {
                                        if ($userfilter == 'passwordexpired') {
    ...
    {
        $dn = "uid=" . $this->getUsername() . "," . static::$ou;
        $data = ["employeetype" => ($leftCampus ? 'leftcampus' : [])];
        if (Ldap::getInstance()->modify($dn, $data)) {
            ...
            return ['RESULT' => true, 'MESSAGE' => 'Leftcampus successfully set.', 'uid' => $this->getUsername()];
        } else {
            return [
                'RESULT' => false,
    ...
    {
        $dn = "uid=" . $this->getUsername() . "," . static::$ou;
        $data = ["employeetype" => ($nonCampus ? 'noncampus' : [])];
        if (Ldap::getInstance()->modify($dn, $data)) {
            ...
            return ['RESULT' => true, 'MESSAGE' => 'Noncampus successfully set.', 'uid' => $this->getUsername()];
        } else {
            return [
                'RESULT' => false,
    ...
    {
        $dn = "uid=" . $this->getUsername() . "," . static::$ou;
        $data = ["employeetype" => ($classroom ? 'classroom' : [])];
        if (Ldap::getInstance()->modify($dn, $data)) {
